...
def define_model(criterion):
    ...
    model = RandomForestClassifier(n_estimators=criterion, n_jobs=8, random_state=42)
    return model
...
def main():
    ...
    for cri in list_cri:
        ...
        str_std = str(std_accuracy) + "_" + str(std_precison) + "_" + str(std_recall) + "_" + str(std_f1) + "_" + str(
            std_auc)
        str_pars = "_" + str(cri) + "_"
        # Ghi Log
        file_result = open(file_name + "_BRF_" + str_mean + str(str_pars) + "_.txt", 'w')
        file_result.write("\n-----------------------------------------------")
        file_result.write('\nFile               ' + str(file_name))
        file_result.write("\n-----------------------------------------------")
        file_result.write('\nMEAN:           |' + str(str_mean))
