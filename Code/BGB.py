...
def define_model(criterion):
    ...
    model = GradientBoostingClassifier(n_estimators=criterion,  random_state=42)
    return model
...
def main():
    ...
    for cri in list_cri:
        ...
        # Ghi Log
        file_result = open(file_name + "_BGB_" + str_mean + str(str_pars) + "_.txt", 'w')
        file_result.write("\n-----------------------------------------------")
        file_result.write('\nFile               ' + str(file_name))
        file_result.write("\n-----------------------------------------------")
        file_result.write('\nMEAN:           |' + str(str_mean))
